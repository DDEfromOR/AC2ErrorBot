    ...
    public class MainDialog : ComponentDialog
    {
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext dialogContext, object options, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (dialogContext.Context.Activity.Type == ActivityTypes.Message)
            {
                ...
                {
                    await SendErrorCardMessage(dialogContext.Context, cancellationToken);
                }
        ...
        private async Task SendErrorCardMessage(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            await turnContext.SendActivityAsync(MessageFactory.Attachment(new CardResource("ErrorOptions.json").AsAttachment()), cancellationToken);
        }
